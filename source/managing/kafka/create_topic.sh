...
################################################################################
# Bash script - create_topic.sh
# Parses a configuration file in order to create topics on a Kafka Cluster.
################################################################################

# Create a potentially replicated topic on the Vagrant virtualized Kafka 
# Cluster.
#
# Arguments:
...
#
# No returned value.
function vagrant_create_topic {
    vagrant ssh "$1" -c "kafka-topics --create --zookeeper ${1}:2181\
...
function create_topic {
    ssh "$1" "kafka-topics --create --zookeeper ${1}:2181\
        --replication-factor ${2} --partition ${3} --topic ${4}"
}
